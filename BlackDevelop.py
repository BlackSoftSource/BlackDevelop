...
from ui import selectprojDialog, newprojDialog, MainWindow, copyingFilesDialog, settingsDialog, projectSettingsDialog, GitHubPushDialog, LoginGitDialog, buildRpmWizard, rpmProgressDialog
...
class SaveAsProjectDialog(QDialog):
    def __init__(self, destination):
        ...
        movie = QMovie("/home/zielak/Dokumenty/Python/BlackDevelop/gifs/copy-file.gif")
...
class CommitGitHub(QDialog):
    ...
    def findCommits(self):
        ...
        try:
            pygit.init(Settings.projectSrc)
        except:
            print("W: Reposiory exists ...")
        ...
        for root, dirs, files in os.walk(Settings.projectPath):
            for file in files:
                ...
                if not len(paths) == 1:
                    ...
                    for data in paths:
                        if p.exists(p.join(Settings.projectPath, data)) and not Settings.projectSrc in p.join(Settings.projectPath, data) and not data == '':
                            newpath = p.join(Settings.projectSrc, p.basename(data))
                            ...
                            Settings.newPaths.append(["./" + p.basename(data), data, file])
                            filedata = filedata.replace(data, "./" + p.basename(data))
...
class SendToGitThread(QThread):
    ...
    def pathFix(self):
        for newoldpath in Settings.newPaths:
            oldpath = newoldpath[0]
            newpath = newoldpath[1]
            file = newoldpath[2]
...
class ImportFromGitHub(QDialog):
    def __init__(self):
        ...
        dialog = QInputDialog()
        repo, ok = dialog.getText(self, 'Import from GitHub', 'Repository:')
        if not ok and repo:
            return None
        appname, ok = dialog.getText(self,'Import from GitHub', 'Program name:')
        if not ok and appname:
            return None
        self.appname = appname
        appver, ok = dialog.getText(self,'Import from GitHub', 'Version:')
        if not ok and appver:
            return None
        ...
        self.ui.label.setText("Importing project from GitHub ...")
        self.ui.progressBar.setMaximum(0)
...
class BuildRpm(QWizard):
    ...
    def loadUi(self):
        ...
        self.setPixmap(QWizard.LogoPixmap, QPixmap("/home/zielak/Dokumenty/Python/BlackDevelop/icons/x-rpm.png"))
...
class RpmBuildProgressDialog(QDialog):
    def __init__(self, parent2):
        ...
        movie = QMovie("/home/zielak/Dokumenty/Python/BlackDevelop/gifs/gear.gif")
